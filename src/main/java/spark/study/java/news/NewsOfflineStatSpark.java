...
public class NewsOfflineStatSpark {
    public static void main(String[] args) {
        ...
        //创建SparkConf和Spark上下文
        SparkConf conf = new SparkConf()
                .setAppName("新闻网站关键指标立离线统计Spark作业")
                .setMaster("local");
    ...
    private static String getYesterday(){
        Calendar cal = Calendar.getInstance();//创建日历实例
        cal.setTime(new Date());//设置当前时间的日历
        cal.add(Calendar.DAY_OF_YEAR , -1);//修改日历的天数，变为昨天
        Date date = cal.getTime();//获取这个日历的时间
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");//解析成我们需要的日期格式的字符串
        return sdf.format(date);//返回的是字符串
    }
    ...
    private static void calculateDailyNewUserRegisterRate(
        ...
        // 执行两条SQL，获取结果
        Object result1 = hiveContext.sql(sql1).collect()[0].get(0);//collect收集到driver端是一个List,然后get获取值
        long number1 = 0L;
        if(result1 != null) {
    ...
    private static void calculateDailyUserJumpRate(
        ...
        // 已注册用户的昨天跳出的总数------就是访问一次就跳出再也没访问（访问次数就一次）
        ...
        DataFrame df = hiveContext.sql(sql1);
        Row row = df.collect()[0];
        Object result1 = row.get(0);
        long number1 = 0L;
        if(result1 != null) {
            number1 = Long.valueOf(String.valueOf(result1));
        }
     ...
     * 格式化小数
     * @param num 格式化的小数
     * @param scale 四舍五入的位数，也就是保留的小数的位数
     * @return 格式化小数
     */
    private static double formatDouble(double num, int scale) {
        BigDecimal bd = new BigDecimal(num);//对精度要求非常高的银行使用这种方式
